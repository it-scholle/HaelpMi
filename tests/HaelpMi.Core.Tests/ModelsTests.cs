...
public class ModelsTests
{
    ...
    [Fact]
    public void TestModeArmState_TryConsume_ReturnsTrue_WhenStillWithinTimeout()
    {
        var state = new TestModeArmState();
        var armedAt = DateTimeOffset.UtcNow;
        state.Arm(armedAt);

        Assert.True(state.TryConsume(armedAt + AppConstants.TestModeTimeout - TimeSpan.FromSeconds(1)));
    }

    [Fact]
    public void TestModeArmState_TryConsume_ReturnsFalse_AfterTimeoutElapsed()
    {
        var state = new TestModeArmState();
        var armedAt = DateTimeOffset.UtcNow;
        state.Arm(armedAt);

        Assert.False(state.TryConsume(armedAt + AppConstants.TestModeTimeout + TimeSpan.FromSeconds(1)));
    }

    [Fact]
    public void TestModeArmState_TryConsume_IsOneShot_SecondCallReturnsFalse()
    {
        var state = new TestModeArmState();
        var armedAt = DateTimeOffset.UtcNow;
        state.Arm(armedAt);

        Assert.True(state.TryConsume(armedAt));
        Assert.False(state.TryConsume(armedAt)); // schon verbraucht - kein zweiter Trigger zaehlt als Test
    }

    [Fact]
    public void TestModeArmState_Disarm_BeforeConsume_PreventsConsumption()
    {
        var state = new TestModeArmState();
        var armedAt = DateTimeOffset.UtcNow;
        state.Arm(armedAt);
        ...
        Assert.False(state.TryConsume(armedAt));
    }

    [Fact]
    public void TestModeArmState_Remaining_ReflectsCountdown_WhileArmed()
    {
        var state = new TestModeArmState();
        var armedAt = DateTimeOffset.UtcNow;
        state.Arm(armedAt);

        var remaining = state.Remaining(armedAt + TimeSpan.FromSeconds(30));

        Assert.NotNull(remaining);
        Assert.Equal(AppConstants.TestModeTimeout - TimeSpan.FromSeconds(30), remaining!.Value);
    }

    [Fact]
    public void TestModeArmState_IsArmed_ReturnsFalse_WhenNeverArmed()
    {
        var state = new TestModeArmState();

        Assert.False(state.IsArmed(DateTimeOffset.UtcNow));
        Assert.Null(state.Remaining(DateTimeOffset.UtcNow));
    }
}
